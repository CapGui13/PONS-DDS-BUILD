...
def dual_chunk(urls, rows):
    assert 1 <= len(rows) <= 24
    mid=(len(rows)+1)//2
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as ex:
        ...
        for url,part in zip(urls,parts):
            if part: fut.append(ex.submit(post,url,part))
...
def main():
    ...
    ap.add_argument('--url1',default='http://127.0.0.1:18086/api/dds')
    ap.add_argument('--url2',default='http://127.0.0.1:18087/api/dds')
    ap.add_argument('--input',required=True)
    ap.add_argument('--expected',required=True)
    ap.add_argument('--stops',required=True)
    ap.add_argument('--output',required=True)
    a=ap.parse_args(); urls=(a.url1,a.url2)
    ...
    assert sorted(by)==list(range(51,101))

    results=[]; logical_total=requested_total=0
    for board in range(51,101):
        logical=stops[board]
        requested=min(120,int(math.ceil(logical/24.0))*24)
        chunk_ms=[]; elapsed=0.0
        for i in range(0,requested,24):
            ms=dual_chunk(urls,by[board][i:i+24]); chunk_ms.append(ms); elapsed+=ms
        logical_total+=logical; requested_total+=requested
        results.append({
            'board':board,
            'logicalStop':logical,
    ...
    vals=[x['elapsedMs'] for x in results]
    report={
        'campaign':'PLAY_R86_R81_EFFECTIVE_HTTP_WORKLOAD_50_DUAL_2X2',
        'status':'PASS',
        'engines':2,
        'threadsPerEngine':2,
        'httpChunkSize':24,
        ...
        'metrics':{
            ...
            'logicalSamples':logical_total,
            'requestedSamples':requested_total,
        },
        'slowest':sorted(results,key=lambda x:x['elapsedMs'],reverse=True)[:10],
