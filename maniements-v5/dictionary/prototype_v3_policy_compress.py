...
def context_json(k):
    ...
    return {'north_remaining':n,'south_remaining':s,'leader':leader,'pos':pos,'won':won,
            'trick_shape':[{'seat':a,'card':b} for a,b in trick]}


def feature_map(r):
    f={}
    for rank in RANKS:
        f[f'W_SEEN_{rank}']=rank in r['west_seen']
        f[f'E_SEEN_{rank}']=rank in r['east_seen']
    f['W_VOID']=bool(r['west_void']); f['E_VOID']=bool(r['east_void'])
    for seat in ('E','W'):
        cur=next((t['card'] for t in r['trick'] if t['seat']==seat),None)
        for rank in RANKS+'-':
            f[f'CUR_{seat}_{rank}']=(cur==rank)
    return f
...
def feature_fr(name):
    ...
    if parts[0] in ('W','E') and parts[1]=='SEEN':
        side='Ouest' if parts[0]=='W' else 'Est'; rank=FR[parts[2]]
        return f'{side} a déjà fourni {rank}'
    if parts[0]=='CUR':
...
def make_tree(rows, features):
    ...
    if best is None:
        return {'error':'UNSEPARABLE','actions':dict(Counter(labels))}
    _,_,feat,left,right=best
    remaining=[x for x in features if x!=feat]
...
def leaf_rules(t,path=None):
    ...
    cond=t['condition_fr']
    return leaf_rules(t['no'],path+[{'condition':cond,'value':False}])+leaf_rules(t['yes'],path+[{'condition':cond,'value':True}])


def main():
    ap=argparse.ArgumentParser(); ap.add_argument('policy'); ap.add_argument('--output')
    a=ap.parse_args(); d=json.loads(Path(a.policy).read_text(encoding='utf-8'))
    ...
    for k,rs in sorted(groups.items(),key=lambda kv:str(kv[0])):
        ...
        item={'context':context_json(k),'state_count':len(rs),'actions':dict(sorted(actions.items()))}
        if len(actions)==1:
            item['unconditional_action']=next(iter(actions)); n=l=1; depth=0
        else:
            conditional+=1
            tree=make_tree([(feature_map(r),r['action']) for r in rs],all_features)
            item['decision_tree']=tree; item['rules']=leaf_rules(tree)
            n,l,depth=tree_stats(tree)
            if any('error' in z for z in item['rules']):errors+=1
        item['tree_stats']={'nodes':n,'leaves':l,'depth':depth}
        total_nodes+=n; total_leaves+=l; max_depth=max(max_depth,depth); contexts.append(item)
    out={'schema':'MANIEMENTS_V5_DICTIONARY_V3_POLICY_PROGRAM_V1','case':d['case'],
         'probability_fraction':d['solve']['probability_fraction'],'native_policy_states':d['policy_state_count'],
         'context_count':len(contexts),'conditional_context_count':conditional,
         'program_stats':{'nodes':total_nodes,'leaves':total_leaves,'max_conditional_depth':max_depth,'unseparable_contexts':errors},
         'contexts':contexts}
